...
player.forEach(coords => {
    Kopal8.Kopaxel.GetKopalxel(coords[0], coords[1]).color = 'BROWN'
})
Kopal8.Kopaxel.GetKopalxel(appleX, appleY).color = 'RED'
Kopal8.Input.Button.onClick(Kopal8.Input.Button.LEFT, () => {
    player.forEach(coords => {
        coords[0] -= 1
        Kopal8.Kopaxel.MoveKopalxel(coords[0] + 1, 34, coords[0], 34)
    })
})
...
Kopal8.Update.OnUpdate(() => {
    if (msPassed % 25 === 0) {
        appleY++;
        Kopal8.Kopaxel.MoveKopalxel(appleX, appleY - 1, appleX, appleY)
        player.forEach(coords => {
            if (appleY == coords[1] && appleX == coords[0]) {
                let nX = Kopal8.Random.inRange(0, 50)
                Kopal8.Kopaxel.MoveKopalxel(appleX, appleY, nX, 0)
                Kopal8.Kopaxel.GetKopalxel(coords[0], coords[1]).color = 'BROWN'
                appleY = 0
                appleX = nX
                score++
